...
export class RevisionContratoComponent {
  ...
  getDocumentoGestorDocumental(documento: DocumentoContrato) {
    this.gestorDocumentalService
      .getDocumento(documento.documento_enlace)
      .subscribe({
        next: (response: any) => {
          if (response.file) {
  ...
  //       await this.uploadAndProcessDocument(this.selectedFile);
  //     }
  //   }
  // }
  ...
  //       } else {
  //         this.selectedFile = null; // Resetea si no es válido
  //         fileName.value = '';
  ...
  //   try {
  //     const base64String: any = await this.fileToBase64(file);
